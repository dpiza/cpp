...
ClapTrap::ClapTrap(std::string name)
	...
	std::cout << "[CT] " << name << " created." << std::endl;
}

ClapTrap::ClapTrap(std::string name, int hp, int ep, int atkDmg)
	: _name(name), _hp(hp), _ep(ep), _atkDmg(atkDmg) {
	std::cout << "[CT] "
			  << "ClapTrap named " << name << " appeared." << std::endl;
}

ClapTrap::ClapTrap(int ep) : _ep(ep) {
	std::cout << "[CT] "
			  << "ClapTrap initialized with ep value: " << ep << std::endl;
}

ClapTrap::ClapTrap(int hp, int atkDmg) : _hp(hp), _atkDmg(atkDmg) {}
...
ClapTrap::~ClapTrap() {
	std::cout << "[CT] " << this->_name << " destroyed." << std::endl;
}
...
void ClapTrap::attack(const std::string &target) {
	if (this->consumeEnergy(1) && this->getHp() > 0) {
		std::cout << "[CT] " << this->_name << " attacks " << target
				  << " causing " << this->_atkDmg << " points of damage!"
				  << std::endl;
	}
}
...
void ClapTrap::beRepaired(unsigned int amount) {
	if (this->consumeEnergy(1) && this->getHp() > 0) {
		...
		std::cout << "[CT] " << this->_name << " was healed for " << amount
				  << " hp." << std::endl;
	}
}
...
void ClapTrap::takeDamage(unsigned int amount) {
	if (this->getHp() > 0) {
		this->setHp(this->getHp() - amount);
		std::cout << "[CT] " << this->_name << " has been hit with " << amount
				  << " atk dmg!" << std::endl;
		if (this->getHp() <= 0) {
